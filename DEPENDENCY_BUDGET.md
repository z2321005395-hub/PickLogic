...
| `pdfrx` 2.4.7 / `pdfium_dart` 0.2.5 / PDFium (accepted) | Pro PDF render, page assembly, text-markup annotation export, and source-preserving edited copies | `pdfrx`/bindings MIT; PDFium and bundled components require 16 notice files | Existing accepted payload only: `pdfium_dart` and `ffi` were already transitive, so direct editing use adds no package or binary | Reusing the current engine is lighter than adding another editor runtime; system open cannot provide embedded editing |
...
| `video_player` 2.13.0 | Shared internal video/audio controls and Android platform playback | BSD-3-Clause | Provisionally estimated 1–4 MiB on Android; measure Debug and arm64 Release user-test-3 artifacts | Direct ExoPlayer channels would duplicate lifecycle, texture, seek, and audio-session behavior |
| `video_player_win` 3.2.2 | Windows Media Foundation implementation of the shared player API | BSD-3-Clause | Upstream reports one approximately 130 KiB plugin DLL; measure Standard/Pro package delta | Direct Media Foundation rendering is not materially lighter after implementing Flutter texture/lifecycle integration |
| `petit_bibtex` 6.2.0 | Parse local BibTeX imports without a custom partial grammar | MIT | Pure Dart; estimated below 200 KiB, and its `petitparser` dependency is already in the resolved graph; measure final Pro delta | A handwritten parser is lighter but fails mature-library correctness expectations for nested and escaped values |

...
| `sqlite3` (candidate) | Direct Dart SQLite access | MIT | Expected low single-digit MB | Manual platform databases increase duplicate code |
| `crypto` (candidate) | Streaming SHA-256 | BSD-3-Clause | Pure Dart, small | `dart:convert` lacks SHA-256 |
...
| PDFium via `pdfrx` (Pro candidate) | Local PDF render, text, thumbnails | zlib/libpng-style package; PDFium third-party notices required | Expected material impact; must measure separately | OS preview lacks consistent text/thumbnail support |

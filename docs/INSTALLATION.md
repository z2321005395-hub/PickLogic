...
These packages are maintainer-validation candidates, not a public or signed release.

## Windows Standard / Pro

1. Download the matching ZIP and `SHA256SUMS.txt` from the private CI artifact or draft release.
...
Do not grant media permission or select a real directory until the maintainer authorizes the read-only validation phase.

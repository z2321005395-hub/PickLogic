...
- Initial environment and nubia Z70 device baseline documentation.
- One Flutter/Dart workspace for Windows Standard, Windows Pro, and Android.
...
- Pro selection translation now repairs common PDF line-break and hyphenation artifacts, accepts bounded selections up to 2,000 characters, and caps public-service fan-out at four concurrent requests.
- The Pro reader now lets users correct extracted source text with automatic retranslation, promote comparison results, lock useful translations while looking up new text, and see the first result without scrolling at compact window heights.
...
- PDF selection translation now offers a one-click, no-key 2,000-character instant engine, exposes the engine selector directly in the right sidebar, and can display bounded alternative translation-memory matches as separate cards; the OpenAI-compatible engine remains optional and advanced.
